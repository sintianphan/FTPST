...
import * as dataFormat from 'pages/CRUD/Salesorderdetails/table/SalesorderdetailsDataFormatters';

// eslint-disable-next-line
import * as itemDataFormat from 'pages/CRUD/Item/table/ItemDataFormatters';

import actions from 'actions/salesorderdetails/salesorderdetailsListActions';
...
const SalesorderdetailsTable = () => {
  ...
  const count = useSelector((store) => store.salesorderdetails.list.count);
  const modalOpen = useSelector((store) => store.salesorderdetails.list.modalOpen);
  const rows = useSelector((store) => store.salesorderdetails.list.rows);
  const idToDelete = useSelector((store) => store.salesorderdetails.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="salesorderdetails" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Salesorderdetails')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/salesorderdetails/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/salesorderdetails/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Salesorderdetails'
                : 'http://localhost:8080/api-docs/#/Salesorderdetails'
            }
          >
            API documentation for salesorderdetails
          </LinkMaterial>
        </div>
      </Widget>
