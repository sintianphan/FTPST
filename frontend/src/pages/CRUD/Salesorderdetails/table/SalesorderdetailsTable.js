...
const SalesorderdetailsTable = () => {
  ...
  const columns = [
    {
      ...
      sortable: false,
      renderCell: (params) =>
        itemDataFormat.listFormatter(params.row[params.field], history, 'item'),
      flex: 1,
  ...
  return (
    <div>
      <Widget
        title={<h4>{humanize('Salesorderdetails')}</h4>}
        disableWidgetMenu
      >
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/salesorderdetails/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
