...
const SalesorderTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Number', title: 'number'},{label: 'Ref', title: 'ref'},
  ...
  const count = useSelector((store) => store.salesorder.list.count);
  const modalOpen = useSelector((store) => store.salesorder.list.modalOpen);
  const rows = useSelector((store) => store.salesorder.list.rows);
  const idToDelete = useSelector((store) => store.salesorder.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="salesorder" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Salesorder')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/salesorder/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/salesorder/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Salesorder'
                : 'http://localhost:8080/api-docs/#/Salesorder'
            }
          >
            API documentation for salesorder
          </LinkMaterial>
        </div>
      </Widget>
