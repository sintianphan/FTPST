...
import * as dataFormat from 'pages/CRUD/Country/table/CountryDataFormatters';

import actions from 'actions/country/countryListActions';
...
const CountryTable = () => {
  ...
  const count = useSelector((store) => store.country.list.count);
  const modalOpen = useSelector((store) => store.country.list.modalOpen);
  const rows = useSelector((store) => store.country.list.rows);
  const idToDelete = useSelector((store) => store.country.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="country" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Country')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/country/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Country'
                : 'http://localhost:8080/api-docs/#/Country'
            }
          >
