...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/country/autocomplete`, { params });
  return response.data;
}

const CountrySelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.country.hasPermissionToCreate,
});

export default connect(select)(CountrySelectItem);
