...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/city/autocomplete`, { params });
...
const select = (state) => ({
  hasPermissionToCreate: state.city.hasPermissionToCreate,
});

export default connect(select)(CitySelectItem);
