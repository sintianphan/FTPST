...
import * as dataFormat from 'pages/CRUD/City/table/CityDataFormatters';

import actions from 'actions/city/cityListActions';
...
const CityTable = () => {
  ...
  const count = useSelector((store) => store.city.list.count);
  const modalOpen = useSelector((store) => store.city.list.modalOpen);
  const rows = useSelector((store) => store.city.list.rows);
  const idToDelete = useSelector((store) => store.city.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="city" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('City')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/city/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/city/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/City'
                : 'http://localhost:8080/api-docs/#/City'
            }
          >
