...
import * as dataFormat from 'pages/CRUD/Item/table/ItemDataFormatters';

// eslint-disable-next-line
import * as itemgroupDataFormat from 'pages/CRUD/Itemgroup/table/ItemgroupDataFormatters';

import actions from 'actions/item/itemListActions';
...
const ItemTable = () => {
  ...
  const [filters, setFilters] = React.useState([
    {label: 'Code', title: 'code'},{label: 'Name', title: 'name'},

          {label: 'Itemgroup', title: 'itemgroup'},
  ]);
  ...
  const count = useSelector((store) => store.item.list.count);
  const modalOpen = useSelector((store) => store.item.list.modalOpen);
  const rows = useSelector((store) => store.item.list.rows);
  const idToDelete = useSelector((store) => store.item.list.idToDelete);
  ...
  const columns = [
      ...
      { field: "itemgroup",
        ...
        sortable: false,
        renderCell: (params) => itemgroupDataFormat.listFormatter(params.row[params.field], history, 'itemgroup'),
        flex: 1,
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="item" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Item')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/item/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Item'
                : 'http://localhost:8080/api-docs/#/Item'
            }
          >
            API documentation for item
          </LinkMaterial>
        </div>
      </Widget>
