...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/item/autocomplete`, { params });
  return response.data;
}

const ItemSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.item.hasPermissionToCreate,
});

export default connect(select)(ItemSelectItem);
