...
import * as dataFormat from 'pages/CRUD/Chartofaccount/table/ChartofaccountDataFormatters';

import actions from 'actions/chartofaccount/chartofaccountListActions';
...
const ChartofaccountTable = () => {
  ...
  const count = useSelector((store) => store.chartofaccount.list.count);
  const modalOpen = useSelector((store) => store.chartofaccount.list.modalOpen);
  const rows = useSelector((store) => store.chartofaccount.list.rows);
  const idToDelete = useSelector((store) => store.chartofaccount.list.idToDelete);
  ...
  const columns = [

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="chartofaccount" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Chartofaccount')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/chartofaccount/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/chartofaccount/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Chartofaccount'
                : 'http://localhost:8080/api-docs/#/Chartofaccount'
            }
          >
            API documentation for chartofaccount
          </LinkMaterial>
        </div>
      </Widget>
