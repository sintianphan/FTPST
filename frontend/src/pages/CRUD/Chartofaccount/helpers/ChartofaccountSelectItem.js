...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/chartofaccount/autocomplete`, { params });
  return response.data;
}

const ChartofaccountSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.chartofaccount.hasPermissionToCreate,
});

export default connect(select)(ChartofaccountSelectItem);
