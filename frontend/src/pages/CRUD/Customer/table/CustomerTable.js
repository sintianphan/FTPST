...
import * as dataFormat from 'pages/CRUD/Customer/table/CustomerDataFormatters';
...
const CustomerTable = () => {
  ...
  const count = useSelector((store) => store.customer.list.count);
  const modalOpen = useSelector((store) => store.customer.list.modalOpen);
  const rows = useSelector((store) => store.customer.list.rows);
  const idToDelete = useSelector((store) => store.customer.list.idToDelete);
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Customer'
                : 'http://localhost:8080/api-docs/#/Customer'
            }
          >
            API documentation for customer
          </LinkMaterial>
        </div>
      </Widget>
