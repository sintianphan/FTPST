...
import * as dataFormat from 'pages/CRUD/Itemgroup/table/ItemgroupDataFormatters';

import actions from 'actions/itemgroup/itemgroupListActions';
...
const ItemgroupTable = () => {
  ...
  const count = useSelector((store) => store.itemgroup.list.count);
  const modalOpen = useSelector((store) => store.itemgroup.list.modalOpen);
  const rows = useSelector((store) => store.itemgroup.list.rows);
  const idToDelete = useSelector((store) => store.itemgroup.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="itemgroup" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Itemgroup')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/itemgroup/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/itemgroup/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Itemgroup'
                : 'http://localhost:8080/api-docs/#/Itemgroup'
            }
          >
            API documentation for itemgroup
          </LinkMaterial>
        </div>
      </Widget>
