...
import * as dataFormat from 'pages/CRUD/Service/table/ServiceDataFormatters';

import actions from 'actions/service/serviceListActions';
...
const ServiceTable = () => {
  ...
  const count = useSelector((store) => store.service.list.count);
  const modalOpen = useSelector((store) => store.service.list.modalOpen);
  const rows = useSelector((store) => store.service.list.rows);
  const idToDelete = useSelector((store) => store.service.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="service" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Service')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/service/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Service'
                : 'http://localhost:8080/api-docs/#/Service'
            }
          >
