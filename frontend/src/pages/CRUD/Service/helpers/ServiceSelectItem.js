...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/service/autocomplete`, { params });
...
const select = (state) => ({
  hasPermissionToCreate: state.service.hasPermissionToCreate,
});

export default connect(select)(ServiceSelectItem);
