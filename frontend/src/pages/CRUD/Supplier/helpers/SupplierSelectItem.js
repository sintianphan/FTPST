...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/supplier/autocomplete`, { params });
  return response.data;
}

const SupplierSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.supplier.hasPermissionToCreate,
});

export default connect(select)(SupplierSelectItem);
