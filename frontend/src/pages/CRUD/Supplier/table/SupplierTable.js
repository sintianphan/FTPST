...
import * as dataFormat from 'pages/CRUD/Supplier/table/SupplierDataFormatters';

// eslint-disable-next-line
import * as cityDataFormat from 'pages/CRUD/City/table/CityDataFormatters';
// eslint-disable-next-line
import * as stateDataFormat from 'pages/CRUD/State/table/StateDataFormatters';
// eslint-disable-next-line
import * as countryDataFormat from 'pages/CRUD/Country/table/CountryDataFormatters';

import actions from 'actions/supplier/supplierListActions';
...
const SupplierTable = () => {
  ...
  const count = useSelector((store) => store.supplier.list.count);
  const modalOpen = useSelector((store) => store.supplier.list.modalOpen);
  const rows = useSelector((store) => store.supplier.list.rows);
  const idToDelete = useSelector((store) => store.supplier.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="supplier" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('Supplier')}</h4>} disableWidgetMenu>
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/supplier/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Supplier'
                : 'http://localhost:8080/api-docs/#/Supplier'
            }
          >
            API documentation for supplier
          </LinkMaterial>
        </div>
      </Widget>
