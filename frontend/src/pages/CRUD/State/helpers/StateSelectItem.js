...
async function selectList(query, limit) {
  ...
  const response = await axios.get(`/state/autocomplete`, { params });
  return response.data;
}

const StateSelectItem = (props) => {
...
const select = (state) => ({
  hasPermissionToCreate: state.state.hasPermissionToCreate,
});

export default connect(select)(StateSelectItem);
