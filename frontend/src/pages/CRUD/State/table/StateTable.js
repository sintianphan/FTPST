...
import * as dataFormat from 'pages/CRUD/State/table/StateDataFormatters';

import actions from 'actions/state/stateListActions';
...
const StateTable = () => {
  ...
  const count = useSelector((store) => store.state.list.count);
  const modalOpen = useSelector((store) => store.state.list.modalOpen);
  const rows = useSelector((store) => store.state.list.rows);
  const idToDelete = useSelector((store) => store.state.list.idToDelete);
  ...
  const columns = [
...

      {
        ...
        flex: 0.6,
        maxWidth: 80,
        renderCell: (params) => <Actions classes={classes} entity="state" openModal={openModal} {...params} />,
      }
  ];

  return (
    <div>
      <Widget title={<h4>{humanize('State')}</h4>} disableWidgetMenu>
        <Box className={classes.actions}>
          <Link to="/admin/state/new">
        ...
        <div style={{minHeight: 500, width: "100%", paddingTop: 20, paddingBottom: 20}}>
          <DataGrid
            ...
            disableColumnMenu
            loading={loading}
            onRowClick={(e) => {history.push(`/admin/state/${e.id}/edit`)}}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/State'
                : 'http://localhost:8080/api-docs/#/State'
            }
          >
