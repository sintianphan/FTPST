...
async function list(filter) {
  const response = await axios.get(
    ...
    &salesorderdetails=${filter.salesorderdetails ? filter.salesorderdetails : ''}
...
async function filterSalesorderdetails(request, filter) {
  const response = await axios.get(`/salesorderdetails?page=${filter.page}&limit=${filter.limit}${request}`);
  return response.data;
}
