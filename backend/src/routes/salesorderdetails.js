...
router.post('/', async (req, res) => {
    await SalesorderdetailsService.create(req.body.data, req.currentUser, true, req.headers.referer);
    const payload = true;
...
router.put('/:id', wrapAsync(async (req, res) => {
  await SalesorderdetailsService.update(req.body.data, req.body.id, req.currentUser);
  const payload = true;
  res.status(200).send(payload);
}));
...
router.get('/', wrapAsync(async (req, res) => {
  const payload = await SalesorderdetailsDBApi.findAll(
    req.query,
  );
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await SalesorderdetailsDBApi.findBy(
    { id: req.params.id },
  );
