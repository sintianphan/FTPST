...
router.post('/', async (req, res) => {
    await SupplierService.create(req.body.data, req.currentUser, true, req.headers.referer);
    const payload = true;
...
router.get('/', wrapAsync(async (req, res) => {
  const payload = await SupplierDBApi.findAll(
    req.query,
  );
...
router.get('/:id', wrapAsync(async (req, res) => {
  const payload = await SupplierDBApi.findBy(
    { id: req.params.id },
  );
